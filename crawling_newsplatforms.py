...
def get_newsplatforms_from_news_title(page_start_num,page_end_num, URL):
    with open('data/[year]/newsplatforms[year].txt', 'w') as filewriter:
        newsplatforms_list = []
...
def count_newsplatform():
    with open('data/[year]/newsplatform[year].txt', 'r') as filereader:
        ...
        sr.to_csv('data/[year]/newsplatform[year]_count.csv')
